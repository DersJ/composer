...
export function useThread(note: Note | null): ThreadResult {
  ...
  useEffect(() => {
    async function fetchParentNotes() {
      if (!ndk || parentNoteIds.length === 0) return;
      ...
      try {
        ...
        // Reverse the array so root notes appear first
        // setParentNotes(notes.reverse());
